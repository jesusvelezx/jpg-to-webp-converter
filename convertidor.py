...
from tkinter import Tk, Frame, Button, Label, Scale, StringVar, DoubleVar, IntVar, HORIZONTAL
...
class ImageConverterApp:
	...
	def crear_interfaz(self):
		...
		ttk.Label(config_frame, text = "Carpeta de origen:").grid(row = 0, column = 0, sticky = "w", pady = 5)
		ttk.Entry(config_frame, textvariable = self.carpeta_origen, width = 40).grid(row = 0, column = 1, sticky = "ew",
		                                                                             pady = 5)
		ttk.Button(config_frame, text = "Examinar", command = self.seleccionar_origen).grid(row = 0, column = 2,
		                                                                                    padx = 5, pady = 5)
		
		ttk.Label(config_frame, text = "Carpeta de destino:").grid(row = 1, column = 0, sticky = "w", pady = 5)
		ttk.Entry(config_frame, textvariable = self.carpeta_destino, width = 40).grid(row = 1, column = 1,
		                                                                              sticky = "ew", pady = 5)
		ttk.Button(config_frame, text = "Examinar", command = self.seleccionar_destino).grid(row = 1, column = 2,
		                                                                                     padx = 5, pady = 5)
		
		# Formato de salida
		ttk.Label(config_frame, text = "Formato de salida:").grid(row = 2, column = 0, sticky = "w", pady = 5)
		formatos = ttk.Combobox(config_frame, textvariable = self.formato_salida, state = "readonly")
		formatos['values'] = ('webp', 'png', 'jpg', 'avif', 'heif')
		formatos.grid(row = 2, column = 1, sticky = "ew", pady = 5)
		formatos.bind("<<ComboboxSelected>>", self.actualizar_opciones_formato)
		
		# Calidad
		ttk.Label(config_frame, text = "Calidad:").grid(row = 3, column = 0, sticky = "w", pady = 5)
		self.scale_calidad = Scale(config_frame, variable = self.calidad_webp, from_ = 0, to = 100,
		                           orient = HORIZONTAL, length = 200)
		self.scale_calidad.grid(row = 3, column = 1, sticky = "ew", pady = 5)
		self.label_calidad = ttk.Label(config_frame, text = "80%")
		self.label_calidad.grid(row = 3, column = 2, sticky = "w", pady = 5)
		self.calidad_webp.trace_add("write", self.actualizar_label_calidad)
		
		# Opciones adicionales
		ttk.Checkbutton(config_frame, text = "Conservar metadatos", variable = self.conservar_metadatos).grid(
				row = 4, column = 0, columnspan = 3, sticky = "w", pady = 5)
		
		ttk.Checkbutton(config_frame, text = "Redimensionar imágenes", variable = self.redimensionar,
		                command = self.toggle_redimension).grid(row = 5, column = 0, columnspan = 2, sticky = "w",
		                                                        pady = 5)
		
		# Frame para opciones de redimensionado (inicialmente oculto)
		self.redim_frame = ttk.Frame(config_frame)
		self.redim_frame.grid(row = 6, column = 0, columnspan = 3, sticky = "w", pady = 5)
		...
		botones_frame.grid(row = 7, column = 0, columnspan = 3, pady = 15)
		
		ttk.Button(botones_frame, text = "Convertir", command = self.iniciar_conversion).pack(side = "left", padx = 5)
		ttk.Button(botones_frame, text = "Vista previa", command = self.mostrar_vista_previa).pack(side = "left",
		                                                                                           padx = 5)
		ttk.Button(botones_frame, text = "Cancelar", command = self.cancelar_conversion).pack(side = "left", padx = 5)
		
		# Barra de progreso
		ttk.Label(config_frame, text = "Progreso:").grid(row = 8, column = 0, sticky = "w", pady = 5)
		self.barra_progreso = ttk.Progressbar(config_frame, variable = self.progreso, length = 300)
		self.barra_progreso.grid(row = 8, column = 1, columnspan = 2, sticky = "ew", pady = 5)
		
		# Estado
		ttk.Label(config_frame, textvariable = self.estado).grid(row = 9, column = 0, columnspan = 3, sticky = "w",
		                                                         pady = 5)
		...
		self.preview_label = ttk.Label(preview_frame, text = "Selecciona un archivo para vista previa")
	...
	def mostrar_vista_previa(self):
		...
		try:
			...
			self.preview_label = Label(self.preview_label.master, image = self.vista_previa_imagen)
	...
	def iniciar_conversion(self):
		...
		if not carpeta_origen or not carpeta_destino:
			messagebox.showwarning("Advertencia", "Debes seleccionar carpetas de origen y destino.")
			return
...
try:
	...
	print("La biblioteca piexif no está instalada. Instalando...")
	import subprocess
...
try:
	...
	print("La biblioteca matplotlib no está instalada. Instalando...")
	import subprocess
